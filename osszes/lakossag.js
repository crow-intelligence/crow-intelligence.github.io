...
scaleX_020301.domain([
    d3.min(categories_020301, function(c) { return d3.min(c.values, function(c) { return c.date; }); }),
    d3.max(categories_020301, function(c) { return d3.max(c.values, function(c) { return c.date; }); }) ]);

scaleY_020301.domain([
    d3.min(categories_020301, function(c) { return d3.min(c.values, function(d) { return d.ydata; }); }),
    d3.max(categories_020301, function(c) { return d3.max(c.values, function(d) { return d.ydata; }); }) ]).nice();
...
legend_020301.append("rect")
    ...
    .attr("x", margin_020301.top + 830)
    .attr("y", function(d, i) {return (i * 20) + 90;} )
    .attr("width", 5)
    .attr("height", 15)
    .style("fill", function(d) {return color_020301(d.name);} );

legend_020301.append("text")
    .attr("x", margin_020301.top + 820)
